...
export async function runAgentLoop({
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction: buildSystemPrompt(prdText, prdMeta),
  });
  ...
  try {
    ...
    try {
      ...
      parsedResponse = {
        drift: { status: "compliant", score: 100, type: "none", reason: "Parsing failed", conflicts_with: "", suggested_alternative: "" },
        ai_review: { status: "compliant", alignment_score: 100, findings: [], missing: [] },
        rule_enforcement: { status: "violation", rules_violated: ["Failed to process rules correctly"], suggested_rewrite: "Please review the issue manually." },
    ...
    await delay(300);
    await broadcast?.({ type: "drift", message: `Drift assessment complete. Score: ${parsedResponse.drift.score}/100`, status: parsedResponse.drift.status, score: parsedResponse.drift.score });

    // Pass 2: AI Review
    await delay(300);
    await broadcast?.({ type: "ai_review", message: "Evaluating strategic alignment...", status: "running" });
    await delay(300);
    await broadcast?.({ type: "ai_review", message: `Alignment score: ${parsedResponse.ai_review.alignment_score}%`, status: parsedResponse.ai_review.status, score: parsedResponse.ai_review.alignment_score });

    // Pass 3: Rule Enforcement
    await delay(300);
    await broadcast?.({ type: "rule_enforcement", message: "Checking formatting rules...", status: "running" });
    await delay(300);
    await broadcast?.({ type: "rule_enforcement", message: `Rule verification complete.`, status: parsedResponse.rule_enforcement.status });

    // Verdict
    await delay(300);
    await broadcast?.({ type: "verdict", message: simulated ? "Dry run complete." : "Applying GitHub actions...", status: "running" });

    const toolContext = { simulated, githubToken, repoFullName, issueNumber, broadcast };
    ...
    return {
      ...parsedResponse,
      toolCalled: parsedResponse.overall?.action || "none",
      ruleQuoted: parsedResponse.rule_enforcement?.rules_violated?.[0] || "",
      confidence: parsedResponse.ai_review?.alignment_score || 0,
      result: parsedResponse.overall?.summary || "",
      iterationCount: 1,
      status: parsedResponse.overall?.status === "violation" ? "violated" : (parsedResponse.overall?.status === "warning" ? "flagged" : "compliant"),
    };
  } catch (error) {
