...
export const agentFunction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const prd = await step.run("fetch-prd", async () => {
      ...
      return { prdText: prdRecord.prdText, prdMeta: prdRecord.prdMeta || {}, userId: prdRecord.userId };
    });
    ...
    if (result.status === "compliant" && !simulated) {
      await step.sendEvent("trigger-code-agent", {
        name: "agent/issue.codegenerate",
        data: {
          repoFullName,
          issueNumber,
          issueTitle,
